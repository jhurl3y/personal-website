...
- **All automated tests — visual regression _and_ unit/component.** Playwright was
...
| `@mui/material-nextjs`              | _(absent)_                 | **added, 9.3.0**    | Required by MUI's documented Pages Router SSR integration. Added in **step 5**.                                                                                                                                                                                      |
| `@emotion/cache`                    | _(absent)_                 | **added, 11.14.0**  | Same — the Emotion cache the App/Document integration wires up.                                                                                                                                                                                                      |
| `@emotion/server`                   | _(absent)_                 | **added, 11.11.0**  | Same — server-side style extraction, replacing the deleted JSS `ServerStyleSheets`.                                                                                                                                                                                  |
...
| `react-transition-group`            | _(undeclared)_             | **added, ^4.4.5**   | **Phantom dependency:** imported directly by `components/Transition/index.js:1` but absent from `package.json`; it resolves today only transitively via MUI. The MUI 5 → 9 rewrite can remove that transitive path and break the build. Must be declared explicitly. |
...
therefore bump MUI within v5 to 5.18.0 _before_ installing React 19, or the install
...
2. **No-op keydown listeners in four files** — `addEventListener("keydown", () => null)`
   ...
   **Explicitly preserved:** `components/Home/index.js:124,128` registers a real named
   ...
   identities. Its _lifecycle_ is correct and must not be swept up in this fix — but its
   _scope_ is not; see 7.11.

3. **The carousel arrow-key handler is bound to `window`** (`components/Home/index.js:122-130`).
...
4. **Transport failures produce an empty error message**
   (`components/Contact/form/index.js:138-149`). Stated accurately after reading the
   code: there _is_ an `else` branch that sets `status` to `"error"` and calls
...
- **Alt text — decided, not left to the implementer.** The image identifiers are
  ...
  - The carousel is **not** purely decorative — the about copy invites the reader to
    ...
    `alt=""` would be wrong.
  - Introduce a typed `HERO_IMAGES` manifest in `constants.tsx`, replacing the two bare
  ...
  - **Design refinement this unlocks.** The photographs are _global_ travel (South
...
- **All `.react-reveal` selectors** — `src/styles.css:1` _and_
...
| Package                  | Version                                                                                                                                                                                                                            |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `typescript`             | **`6.0.3`** — _not_ 7.x. `typescript-eslint@8.66.0` declares `typescript: ">=4.8.4 <6.1.0"`; pinning TypeScript 7.0.2 is a hard peer conflict and the parser does not support it. Revisit when typescript-eslint widens the range. |
| `eslint`                 | `10.8.0`                                                                                                                                                                                                                           |
...
1. _Typecheck._ `@types/react` 19.x and `eslint-config-next` 16 presuppose the React 19 /
...
2. _Lint during build._ **Next 14 runs ESLint as part of `next build`**, and build-time
...
2. Dead code removal (section 9); drop `react-twitter-embed` and `react-cookie-consent`
   ...
   **`lodash.chunk`, `lodash.shuffle` and `smoothscroll-polyfill` still have live call
   ...
   the build. Their replacements must land in _this same commit_: local `chunk`/`shuffle`
...
3. Bug fixes that are stack-independent: **7.1–7.4, 7.6, 7.11, 7.12, and the behavioural half of 7.10**.
   **7.5 is deliberately excluded** and lands in 4a instead — moving `library.add()` to
   ...
   the existing stack. The _typed_ half of 7.10 — representing absence as a real state
...
Fixes 7.7, 7.8 and 7.9 are deliberately _not_ in step 3. An earlier draft claimed all
...
| 7b  | Network failure / offline (XHR status 0, or `onerror` fires) | The **generic fallback message** renders — not a blank error box — because `responseText` is empty on this path. Form stays populated and re-submittable. (The error _branch_ already exists; what is being verified is that it says something.) | **7.12**              |
...
3. ~~**Hero image descriptions**~~ — **RESOLVED.** All 8 written; see the manifest in
...
4. ~~`getAge()` birth date location~~ — resolved. The date is not in `constants.js`;
   ...
   moves into `constants.tsx` so it is not buried in a component.
